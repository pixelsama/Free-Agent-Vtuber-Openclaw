...
class Config:
    def __init__(self):
        # 从环境变量读取，找不到再退回占位符
        app_key = os.getenv("ASR_VOLC_APP_KEY", "xxxxxxx")
        access_key = os.getenv("ASR_VOLC_ACCESS_KEY", "xxxxxxxxxxxx")
        resource_id = os.getenv("ASR_VOLC_RESOURCE_ID", "volc.bigasr.sauc.duration")
        self.auth = {
            "app_key": app_key,
    ...
    def convert_wav_with_path(audio_path: str, sample_rate: int = DEFAULT_SAMPLE_RATE) -> bytes:
        try:
            ...
            try:
                ...
                for packet in container.demux(audio_stream):
                    for frame in packet.decode():
                        for rframe in resampler.resample(frame):
                            ...
                            pcm_bytes.extend(arr.tobytes(order='C'))
                ...
                logger.error(f"PyAV conversion failed: {e}")
                raise RuntimeError(f"Audio conversion failed via PyAV: {e}")
...
async def main():
    ...
    parser.add_argument("--url", type=str, default=os.getenv("ASR_VOLC_ENDPOINT", "wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_async"), 
                       help="WebSocket URL")
